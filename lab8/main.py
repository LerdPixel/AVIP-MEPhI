...
def save_ngldm_png(S: np.ndarray, path: str, cmap="hot"):
    """
    Сохраняет матрицу NGLDM как изображение с цветовой шкалой.
    Используется логарифмическая нормализация для видимости деталей.
    """
    S_disp = np.log1p(S)  # log(1 + x) защищает от log(0)

    plt.figure(figsize=(6, 5))
    im = plt.imshow(S_disp, cmap=cmap, aspect="auto")
    plt.colorbar(im, label="log(1 + значение)")
    plt.title("Матрица NGLDM")
    plt.xlabel("")
    plt.ylabel("Уровни яркости (кванты)")
    plt.tight_layout()
    plt.savefig(path)
    plt.close()
"""
...
def compute_ngldm(L_gray: np.ndarray, d=1, G=16, save_path: str = None):
    bins = np.linspace(0, 256, G + 1)
    Q = np.digitize(L_gray, bins) - 1
    Q = np.clip(Q, 0, G - 1)
    ...
    for y in range(h):
        for x in range(w):
            ...
            for dy, dx in offsets:
                ny, nx = y + dy, x + dx
                if 0 <= ny < h and 0 <= nx < w and Q[ny, nx] == g:
                    count += 1
            S[g, count] += 1

    # Сохраняем в CSV, если указан путь
    if save_path:
        with open(save_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([f"dep={i}" for i in range(S.shape[1])])
            for row in S:
                writer.writerow(row)
...
def process_image(path: str):
    ...
    save_ngldm_png(S_ngl, os.path.join(DST_DIR, f"{name}_ngldm.png"))
    save_ngldm_png(S_ngl_eq, os.path.join(DST_DIR, f"{name}_ngldm_eq.png"))
